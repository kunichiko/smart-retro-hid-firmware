...
#endif
					if( ctx->endpoint_mode[ep] == 2 )
					{
						// Bidirectional with AUTO_TOG: no manual toggle
						if( len )
						{
							if( len < 0 ) len = 0;
							UEP_CTRL_LEN( ep ) = len;
		...
		case CUIS_TOKEN_OUT:
			...
			switch( ep )
			{
				...
				default:
					if( ctx->endpoint_mode[ep] != 2 )
					{
						// Non-bidirectional: manual toggle
#if defined (CH5xx) || defined (CH32X03x) || defined (CH32V10x)
					...
					// Bidirectional (mode==2): AUTO_TOG handles toggle
#if FUSB_USER_HANDLERS
...
#if USBFS_EP1_MODE == 2
	// Bidirectional: TX + RX both enabled
	USBFS->UEP4_1_MOD = USBFS_UEP1_TX_EN | USBFS_UEP1_RX_EN;
#elif USBFS_EP1_MODE > 0
...
#if USBFS_EP2_MODE == 2
	// Bidirectional: TX + RX both enabled
	USBFS->UEP2_3_MOD = USBFS_UEP2_TX_EN | USBFS_UEP2_RX_EN;
#elif USBFS_EP2_MODE > 0
...
#error You must have at least EP0!
	...
	{
		if( USBFSCTX.endpoint_mode[i] == 2 )
		{
			// Bidirectional: AUTO_TOG + TX NAK + RX ACK
#if defined (CH5xx) || defined (CH32X03x) || defined (CH32V10x)
			UEP_CTRL_TX(i) = RB_UEP_AUTO_TOG | USBFS_UEP_T_RES_NAK | USBFS_UEP_R_RES_ACK;
#else
